Shader "Universal Render Pipeline/Custom/ToonShader_pointlight"
{
    Properties
    {   
        [Header(Texture)]
        _BaseMap ("基础色", 2D) = "white" {}
        _BaseMap_ST ("基础色 Tiling/Offset", Vector) = (1,1,0,0)  // 添加这一行
        _ao ("ao",2D) = "gray"{}
        ...
        _PointLightIntensity("点光源强度", Range(0, 5)) = 1.0
        ...
        Pass
        {
            ...
            half4 frag (Varyings input) : SV_Target
            {
                // 检查是否有主光源
                    ...
                    bool hasMainLight = mainLight.distanceAttenuation > 0;
                #else
                ...
                if (hasMainLight)
                {
                    ...
                    shadowAtten = saturate(shadowAtten * 2.0);
                    ...
                    half final_toon = toon_diffuse * toon_middiffuse * shadowAtten * distanceAtten;
                    ...
                    half3 spec = saturate(ndotl * spec_color * _specInt) * shadowAtten;
        
                    // 主光源贡献
                    final += finalBaseCol * final_toon * mainLight.color + spec;
                }
                else
                {
                    // 没有主光源时使用环境光作为基础照明
                    final += finalBaseCol * 0.1; // 基础环境光
                }
                ...
                for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
                {
                    ...
                    if (additionalLight.distanceAttenuation > 0.0)
                    {
                        float3 addLightDir = normalize(additionalLight.direction);
            
                        // 点光源漫反射
                        half add_ndotl = dot(normalDir, addLightDir);
                        half add_toon_diffuse = lerp(_colorA, 1.0, saturate(add_ndotl * _Toon_Hardness));
                        half add_toon_middiffuse = lerp(_colorB, 1.0, saturate((add_ndotl - _Toon_Theshold) * _Toon_Hardness));
                        half add_final_toon = add_toon_diffuse * add_toon_middiffuse;
            
                        // 点光源镜面反射
                        float3 add_half_dir = normalize(addLightDir + view_dir);
                        half add_ndoth = dot(normalDir, add_half_dir);
                        half3 add_spec_color = step(0.8, pow(max(0.0, add_ndoth), Pow_spec));
                        half3 add_spec = saturate(add_ndotl * add_spec_color * _specInt);

                        float lightIntensity = _PointLightIntensity;
            
                        // 应用点光源阴影
                        half addShadowAtten = additionalLight.shadowAttenuation;
                        final += (finalBaseCol * add_final_toon * additionalLight.color + add_spec) * 
                                 additionalLight.distanceAttenuation * lightIntensity * addShadowAtten;
                    }
                }
                #endif
